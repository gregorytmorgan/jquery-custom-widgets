...
module.exports = function(config) {
    config.set({
        ...
        reporters: ['progress'], // remove 'progress' to cleanup output, add for debugging
...
//        browsers: ['Chrome', 'Firefox'],
        ...
        client: {
          ...
          qunit: {
            showUI: false,
            testTimeout: 5000,
            autostart: true, // 'true' required unless the tests/testrunner explicitly calls start()
            notrycatch: false // 'false' is necessary to deal with multiple calls to start() ???
          }
        } // client
    }); // config.set
};